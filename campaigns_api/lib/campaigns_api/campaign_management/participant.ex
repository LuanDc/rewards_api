defmodule CampaignsApi.CampaignManagement.Participant do
  ...
  Participants belong to a product and can be associated with multiple campaigns and challenges.
  ...
  @derive {Jason.Encoder,
           only: [:id, :name, :nickname, :product_id, :status, :inserted_at, :updated_at]}
  ...
  def changeset(participant, attrs) do
    participant
    |> cast(attrs, [:product_id, :name, :nickname, :status])
    |> validate_required([:product_id, :name, :nickname])
